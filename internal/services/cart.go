...
type CartService struct {
	...
	sales map[string]models.SalesSummary
}

func NewCartService() *CartService {
	return &CartService{carts: map[string]*cart{}, sales: map[string]models.SalesSummary{}}
}
...
func (s *CartService) ensure(session string) *cart {
	s.mu.Lock()
	defer s.mu.Unlock()
	if _, ok := s.carts[session]; !ok {
		s.carts[session] = &cart{items: map[string]*models.CartItem{}}
	}
...
func (s *CartService) ChangeQty(session, key string, delta int) {
	...
	if it, ok := c.items[key]; ok {
		...
		it.Subtotal = it.Quantity * it.MenuItem.Price
	}
}

func (s *CartService) Remove(session, key string) {
	c := s.ensure(session)
	s.mu.Lock()
	defer s.mu.Unlock()
	delete(c.items, key)
...
func (s *CartService) Snapshot(session string) ([]models.CartItem, int) {
	...
	s.mu.RLock()
	defer s.mu.RUnlock()
	items := make([]models.CartItem, 0, len(c.items))
	total := 0
...
func (s *CartService) Clear(session string) {
	c := s.ensure(session)
	s.mu.Lock()
	defer s.mu.Unlock()
	c.items = map[string]*models.CartItem{}
}

func (s *CartService) RecordSale(session string, total int) {
	s.mu.Lock()
	defer s.mu.Unlock()
	summary := s.sales[session]
	summary.Count++
	summary.Total += total
	s.sales[session] = summary
}

func (s *CartService) SessionSales(session string) models.SalesSummary {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.sales[session]
}
...
func (s InvoiceService) Get(id int) (models.Invoice, error) {
	...
	if err := s.DB.QueryRow("SELECT id, created_at, total FROM invoices WHERE id = ?", id).Scan(&inv.ID, &inv.CreatedAt, &inv.Total); err != nil {
		return inv, err
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&it.ItemName, &it.Quantity, &it.UnitPrice, &it.Subtotal); err != nil {
			return inv, err
		}
...
func (s InvoiceService) TodaySales() (models.SalesSummary, error) {
	var summary models.SalesSummary
	err := s.DB.QueryRow("SELECT COUNT(*), COALESCE(SUM(total), 0) FROM invoices WHERE DATE(created_at, 'localtime') = DATE('now', 'localtime')").Scan(&summary.Count, &summary.Total)
	return summary, err
}
